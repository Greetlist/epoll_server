...
int VecBuffer::ReadFromFd(int fd) {
  ...
  read_index_ += OnMessage(buffer_ + read_index_, n_read);
}
...
int VecBuffer::SaveData(char* data, int data_len) {
  ...
  if (left_data_size > data_len) {
    ...
    EnlargeBufferAndMoveData(cur_buffer, data + left_data_size, data_len - left_data_size);
  } else {
...
int VecBuffer::GetConsumableNum(char* buf) {
  return write_index_ - read_index_;
}

int VecBuffer::GetCurrentDataNum() {
  return write_index_;
}
